...
FigureS3H <- function(p2glinks, xlabels, ylabels, col1, col2, width, height) {
  ...
  df_pos_tab <- df_pos_tab %>% mutate(., 
                                      Percentage = round(Freq / sum(Freq) * 100))
  df_neg <- data.frame(Domain = p2glinks$negCor$domain, Correlation = rep("Negative", length(p2glinks$negCor)))
  df_neg_tab <- as.data.frame(table(df_neg)) 
  df_neg_tab <- df_neg_tab %>% mutate(., 
                                      Percentage = round(Freq / sum(Freq) * 100))
  df_control <- data.frame(Domain = p2glinks$noCor$domain, Correlation = rep("Control", length(p2glinks$noCor)))
  df_control_tab <- as.data.frame(table(df_control)) 
  df_control_tab <- df_control_tab %>% mutate(., 
                                              Percentage = round(Freq / sum(Freq) * 100))
  DF <- rbind(df_pos_tab, df_neg_tab, df_control_tab)
  
  p <- ggplot(DF, aes(x = Correlation, y = Percentage)) + 
    geom_bar(aes(fill = Domain), stat = 'identity') + 
    scale_fill_manual("", values = c('interTAD' = col1, 'intraTAD' = col2)) +
    xlab(xlabels) + 
    ylab(ylabels)
  
  f.table_pos <- rbind(c(sum(df_pos$Domain=='intraTAD'), sum(df_pos$Domain=='interTAD')),
                       c(sum(df_control$Domain=='intraTAD'), sum(df_control$Domain=='interTAD')))
  f.table_neg <- rbind(c(sum(df_neg$Domain=='intraTAD'), sum(df_neg$Domain=='interTAD')),
                       c(sum(df_control$Domain=='intraTAD'), sum(df_control$Domain=='interTAD')))
  fisher_df <- data.frame(group1=c('Positive','Negative'),group2=c('Control','Control'),y.position=c(105,115))
  fisher_df$p <- as.numeric((scientific(c(fisher.test(f.table_pos)$p.value,fisher.test(f.table_neg)$p.value),digits = 2)))
  fisher_df$p[fisher_df$p>2e-16] <- paste0('p=',fisher_df$p[fisher_df$p>2e-16])
  fisher_df$p[fisher_df$p<2e-16] <- 'p<2.2e-16'
  p <- p + stat_pvalue_manual(data=fisher_df) + theme(legend.position = 'top') + scale_y_continuous(limits=c(0,115),breaks = c(0,25,50,75,100))
  pdf('plots/figures/FigureS3H.pdf', height=height, width = width)
  print(p)
  dev.off()
...
motifs <- Figure3C_1(mat_f=read.table('results/P2G_binaryMat_posCor.tsv',header=T),expr=expr[rowMaxs(expr)>=6,],
...
o <- Figure3D(mat=p2glinks$posCor,anno_mat=read.table('results/P2G_binaryMat_posCor.tsv',header=T),which_clusters=c("NSC","IPC","PN1","PN2"),
         mat_f=as.data.frame(readRDS('results/Monocle3_int/pd_P2G_GAMmat.RDS')),
         mat_rna=as.data.frame(readRDS('results/Monocle3_int/pd_genes_GAMmat.RDS')),
...
Figure3E(PWM='results/scATAC/combined_pwm.RDS', 
         scRNA_expression=expr,
         posCOR_pairs='results/P2G_binaryMat_posCor.tsv',
         pseudotime='results/PD_Enh_RNA.tsv',
         FDR=1,out_f='Figure3E',
         expression_cutoff=3,height=12,width=8)

Figure3F(out_f='Figure3F',PWM='results/scATAC/combined_pwm.RDS',expression_cutoff=3,
         pseudotime='results/PD_Enh_RNA.tsv',
         motif_name="Neurog2(var.2)",chip_f="/home/hpc/bonev/data/hic/data/peaks/final/NPC_Neurog2.bed",
         posCOR_pairs='results/P2G_binaryMat_posCor.tsv',FDR_cutoff=0.2, width=6,height=8)

Figure3G(object=coembed.object,mat=read.table('results/P2G_binaryMat_posCor.tsv',header=T),
...
mat <- Figure3A(selected_peaks=p2glinks$noCor,scaleMax=F,seRNA=seRNA_all,sePB=sePB_all,out_f='FigureS3E',
                which_clusters_rna = c('NSC','IPC','PN1','PN2','PN3'),which_clusters = c('NSC','IPC','PN1','PN2'),
                features=NULL,
                cols1=colorRamp2(seq(-2,2,length.out=length(heatmap_colors)), heatmap_colors),cluster_cols1=RNA_cluster_colors[-c(2,4)],cluster_cols2=RNA_cluster_colors[-c(2,4)],
                cols2=colorRamp2(seq(-2,2,length.out=9), rev(colorpalette('rdbu',9))),anno.size=12,height=12,width=10)
write.table(mat,file = paste0('results/P2G_binaryMat_noCor.tsv'),quote = F,col.names=T,row.names=F,sep='\t')
mat <- Figure3A(selected_peaks=p2glinks$negCor,scaleMax=F,seRNA=seRNA_all,sePB=sePB_all,out_f='FigureS3F',
                which_clusters_rna = c('NSC','IPC','PN1','PN2','PN3'),which_clusters = c('NSC','IPC','PN1','PN2'),
                features=NULL,
                cols1=colorRamp2(seq(-2,2,length.out=length(heatmap_colors)), heatmap_colors),cluster_cols1=RNA_cluster_colors[-c(2,4)],cluster_cols2=RNA_cluster_colors[-c(2,4)],
                cols2=colorRamp2(seq(-2,2,length.out=9), rev(colorpalette('rdbu',9))),anno.size=12,height=12,width=10)
...
FigureS3J(mat=p2glinks,cols=c('darkred','darkblue','grey'),height=6,width=3.5)
...
Figure3D_mod <- function(mat,mat_expr,mat_prom_acc,anno_mat,which_clusters=NULL,object=coembed.object,features,cols1,cols2,cluster_cols,anno.size,width,height){
  pd <- object$pseudotime
...
motif_footprinting <- function(archr_obj=archr_obj,flank=500,motif=NULL,regions=NULL,normalize_by='Subtract',file_f,which_clusters=levels(atac.object),cols=cluster_colors(length(levels(atac.object))),plot_bias=T,anno.size=12,key.size=4,width,height){
...
o <- Figure3D_mod(mat=p2glinks$posCor,anno_mat=read.table('results/P2G_binaryMat_posCor.tsv',header=T),which_clusters=c("NSC","IPC","PN1","PN2"),
...
p <- VlnPlot(rna.object, features = c('Eomes'),cols=RNA_cluster_colors[c(1,3,5:7)],idents = c('NSC','IPC','PN1','PN2','PN3'),pt.size = 0,combine = F,same.y.lims = F)
p1 <- p[[1]] + theme(legend.position='none',axis.text.x = element_text(angle = 0, hjust = 0.5))+ xlab('')+ ggtitle('Eomes') + ylab('Normalized expression')
...
pdf('plots/figures/Eomes_rnaExpr.pdf',width=5,height=4)
print(p1)
dev.off()
...
pdf('plots/figures/FigureS3K_2.pdf',width=9,height=6)
print(p1+p2)
dev.off()
...
pdf('plots/figures/FigureS3K_3.pdf',width=9,height=6)
print(p1+p2)
dev.off()
...
motif_footprinting(archr_obj=archr_obj,flank=500,motif='Tead2',regions=NULL,
                   normalize_by='Subtract',file_f='FigureS3L_2',cols=ATAC_cluster_colors[1:4],
                   which_clusters=c('NSC','IPC','PN1','PN2'),plot_bias=T,anno.size=12,key.size=4,width=6,height=6)

motif_footprinting(archr_obj=archr_obj,flank=500,motif='Fos..jun.var.2_327',regions=NULL,
                   normalize_by='Subtract',file_f='FigureS3L_3',cols=ATAC_cluster_colors[1:4],
                   which_clusters=c('NSC','IPC','PN1','PN2'),plot_bias=T,anno.size=12,key.size=4,width=6,height=6)

motif_footprinting(archr_obj=archr_obj,flank=500,motif='Ctcf',regions=NULL,
